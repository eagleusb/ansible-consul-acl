...
def decode_rules_as_yml(rules_as_yml):
    ...
    if rules_as_yml:
        for rule_as_yml in rules_as_yml:
            ...
            for scope in RULE_SCOPES:
                if scope in rule_as_yml:
                    if rule_as_yml[scope] is None:
                        raise ValueError(
                            "Rule for '%s' does not have a value associated to the scope" % scope)
            ...
            if not rule_added:
                raise ValueError(
                    "A rule requires one of %s and a policy." % ('/'.join(RULE_SCOPES)))
    return rules
...
class Configuration:
    ...
    def __init__(self, management_token=None, host=None, scheme=None, validate_certs=None, name=None, port=None,
                 rules=None, state=None, token=None, token_type=None, policy=None):
...
class RuleCollection:
    ...
    def add(self, rule):
        ...
        if rule.pattern in self._rules[rule.scope]:
            ...
            raise ValueError("Duplicate rule for scope '%s'%s" %
                             (rule.scope, patten_info))
...
def main():
    ...
    return_values = dict(changed=output.changed,
                         token=output.token, operation=output.operation)
